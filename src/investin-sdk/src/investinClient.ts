...
export class InvestinClient {
  ...
  private async fundMarginData(fund): Promise<{ balance: number }> {
    ...
    // let mangoAccounts = await this.mangoClient.getMangoAccountsForOwner(mangoGroup, fund.fund_pda);
    // debugger
    try {
  ...
  private async getFundData(data, platformData, prices?: COINGECKO_TOKEN[]): Promise<FUND | undefined> {
    ...
    // handle multiple fund accounts being returned for a sinlge fund
    let decodedData: any;
    if (Array.isArray(data)) {
      for (let i = 0; i < data.length; i++) {
        try {
          decodedData = FUND_DATA.decode(data[i].account.data);
          break;
        } catch {}
      }
    } else {
      decodedData = FUND_DATA.decode(data.account.data);
    }
    if (!decodedData) throw new Error('Can not decode fund account data');
  ...
  async findOrRetrieveFund(managerAccount: PublicKey) {
    ...
    if (!fund) {
      ...
      fund = await this.getFundData(fundData, platformData);
